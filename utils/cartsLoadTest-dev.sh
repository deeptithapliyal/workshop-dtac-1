...
export CARTS_PODS_AVAILABLE=$(kubectl get deployments/carts -n dev -o json | jq '.status.readyReplicas')
...
do
    ...
    sleep 30
    CARTS_PODS_AVAILABLE=$(kubectl get deployments/carts -n dev -o json | jq '.status.readyReplicas')

    if [ "$CARTS_PODS_AVAILABLE" -eq 1 ]
    then
        echo $CARTS_PODS_AVAILABLE
        echo "Beginning Load Test in dev"
    else
        echo $CARTS_PODS_AVAILABLE
        echo "Checking again"
    fi
